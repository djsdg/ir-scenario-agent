...
- `update_scenario` / `update_use_case`；
- `transition_record`；
- `move_use_case`；
- `save_memory`。

匹配、查询、草稿和 `validate_library` 工具是只读的。拒绝审批时不会修改场景库；批准、拒绝、耗时和结果会写入审计日志。

写入前可以调用 `validate_library` 做一次质量审计。它会检查重复 SC/UC ID、SC→UC 引用不存在、孤儿 UC、UC 多父场景、IR 追溯断链，以及当前 Spec 要求的 SC/UC 必填字段。审计失败只会返回问题清单，不会自动修复数据。
...
- 默认检索是关键词证据 + TF-IDF + Spec 同义词扩展；没有配置 Embedding 时，隐含语义覆盖仍然有限；
...
- SQLite 后端提供事务、WAL 和过期快照保护，但还没有企业级角色权限、审计查询服务或 PLM 远程同步；JSON/目录库适合单机或低并发场景。
...
1. **检索增强**：继续校准关键词/TF-IDF/Embedding 权重，并增加领域 Reranker 和真实标注集；
...
7. **服务化部署**：使用可选 REST API 接入 Web/PLM，生产环境补充 HTTPS、角色权限、限流和审计查询。
